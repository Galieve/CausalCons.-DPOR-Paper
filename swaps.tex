...
Simply extending histories according to $\genericNext$ and making recursive calls on re-ordered histories whenever possible (taking $\genericProtocol$ as $\mathit{true}$) guarantees soundness and completeness, but it does not guarantee optimality. Intuitively, the source of redundancy is related to the fact that applying $\genericSwap$ on different histories may give the same result. 
...
\begin{cframed}[pinegreen]
	\begin{equation*}
		\begin{array}{l}
			\genericProtocol(h_<, r, t) \coloneqq 
%			\begin{array}{l}
