...
Our tool takes as input a Java program and isolation levels as parameters. We assume that the program uses a fixed API for interacting with the database, similar to a key-value store interface. This API consists of specific methods for starting/ending a transaction, reading/writing a global variable. The fixed API is required for being able to maintain the database state separately from the JVM state (the state of the Java program) and update the current history in each database access. This relies on a mechanism for ``transferring'' values read from the database state to the JVM state.
...
\begin{figure}[t]
	...
	\begin{subfigure}[t]{0.49\linewidth}
		...
		\caption{Number of histories. For the eight benchmark, we find $12433$ consistent histories under $\RA$, $2309$ under $\CC$ and $1605$ under $\SI$ (out of the figure). \\\\}
...
\begin{figure}[t]
	...
	\begin{subfigure}[t]{0.49\linewidth}
		...
		\caption{Execution time. For the last two heavy clients, the running time is $10' 58''$ and $29' 31''$ (out of the figure).}
