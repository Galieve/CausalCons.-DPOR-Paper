...
\begin{corollary}
	...
	Let $h_< = \swap((h_p, <_{h_p}), r, t)$, where $h_p$ is a reachable history. If $\genericProtocol((h_p, <_{h_p}), r, t)$ holds, then also $\swapped{h'}{r}$ does.
\end{corollary}
\begin{proof}
	As the last event in $h$ is $r$ and by definition of $\swap$ function no event reads from $r$ in $h$ besides $r$, to prove $\swapped{h}{r}$ holds we just need to show that $t >_{\ora} r$. By lemma \ref{lemma:reachable-or-respectful}, $\oraRespectful{h_p}{<_{h_p}}$ holds. As $r <_{h_p} t$, $\genericProtocol((h_p, <_{h_p}), r, t)$ holds and $t$ is $(\so \cup \wro)^+$-maximal, we conclude that $r <_{\ora} t$.
\end{proof}
