...
commit
	...
	\caption{A history and one of its prefixes. The last picture does not correspond to a prefix as some transactions read from the missing one.}
\end{figure}
...
commit
	...
	\begin{subfigure}[b]{.22\textwidth}
		\resizebox{\textwidth}{!}{
			\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
				...
				{\begin{tabular}{l} 
						$\rd{y}$ \\ {\pgfsetfillopacity{0.3}$\rd{x}$}
				\end{tabular}};			
	...
	\caption{Two causal extensions of a history $h$. The extension of $h$ where $t_4$ reads $x$ from $t_2$ and $y$ from $t_1$ is not included as it is not a causal extension ($(t_2, t_4) \not\in \so \cup \wro$). Events in gray are not yet added to the history. }
\end{figure}
...
\caption{A bi-threaded program and a $\SER$/ $\SI$-consistent history. Events in gray are not yet added to the history. All $\so$-edges from $\init$ to the other transactions are omitted for legibility.}
...
In Figure~\ref{fig:non-causally-extensible} we exhibit why $\SI$ and $\SER$ are not causally extensible. For the program in Figure~\ref{fig:non-causally-extensible:prog-ser}, the history presented in Figure~\ref{fig:non-causally-extensible:ser} is $\SER$-consistent (and therefore $\SI$-consistent) but whose only extension (Figure~\ref{fig:non-causally-extensible:ser-cont}) is $\SI$-inconsistent (and therefore $\SER$-inconsistent).
