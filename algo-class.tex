...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\ForAll{$w \in \textbf{\genericValidWrites}(h,e)$} %\writeVar{w}{x}
		...
		\If{$\textbf{\genericProtocol}(h \bullet e, \alpha, \beta)$}
		\State $\textsc{explore}(\textbf{\genericSwap}(h \bullet e, \alpha, \beta))$	
...
\begin{itemize}
	...
	\item $\genericValidWrites: \mathcal{H}_\mathcal{P} \times \mathcal{E}_\mathcal{P} \to \mathcal{E}_\mathcal{P}^*$, where for every $e' \in \genericValidWrites(h, e)$, $e' \in h$ and $h \bullet_{e'} e$ is consistent, %\textcolor{red}{this is for determine when computing a total inconsistent history shall be taken into account}
