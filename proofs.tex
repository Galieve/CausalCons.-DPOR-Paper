...
\begin{lemma}
\label{lemma:recursiveIterativeSame}
Let $h$ a consistent incomplete history and $s$ a state of the program such that $h$ appears at line \ref{algorithm:stmc:outer_loop}. Let $h'$ the next history computed by the algorithm from $s$. Then $\prev(h') = h$.
\end{lemma}
...
\begin{itemize}
	...
	\item \underline{Swapping $r \in h$ and $w \in \tr(e)$:} As $\protocol(h, r, w)$ is satisfied and $h$ is $\ora$-respectful, for every event $e'$ and transaction $T$, $\min_{\ora} \dep(h_p, T, e') = \min_{\ora} \dep(h, T, e')$, so for every pair of transactions $\minimalDependency(h_p, T, T', \bot) = \minimalDependency(h, T, T', \bot)$. In particular, this implies $T \leq^{h_p} T'$ if and only if $T \leq^h T'$ for every  pair $T, T'$ and $T \leq^h \tr(r)$; so $\leq^h \equiv \leq_h$. 
\end{itemize}
\end{proof}
\end{lemma}
...
\begin{lemma}
Let $h$ a history computed by the algorithm \ref{algorithm:stmc}. If $h$ appears at line \ref{algorithm:stmc:outer_loop} when the state is $s$, both $h$ and $s$ will reappear paired at line \ref{algorithm:stmc:inner_loop}.
\end{lemma}

%This number will be called $\Delta(h,s)$.
%, when $\Delta(s,h) = 0$,
\begin{proof}
This proof is made by induction on the number of pairs $(h_i, s_i)$ the algorithm computes starting with $(h,s)$ at line \ref{algorithm:stmc:outer_loop} until reaching some history inconsistent or complete. Let's call $\Delta(h, s)$ to this number. First of all, let's notice that $\Delta(h,s) < \omega$ as the number of events is finite. %and for every type of $a$ distinct of $\bot$, the number of 

\underline{Base case: $\Delta(h,s) = 0$} This case is trivial by the algorithm's definition.

\underline{Inductive case: for every $h,s$ such that $\Delta(h,s) \leq n$ the lemma holds.} Let $h,s$ such that $\Delta(h,s) = n+1$, $a = \next(h,s)$ and $h' = h \bullet a$. Here we have to distinguish between the type of $a$:
\begin{itemize}
\item \underline{$\ibegin$, $\iwrite$}: 
\end{itemize}


%Therefore, the base case follows trivially but the inductive case depends on the type of $a = \textsc{next}(h)$ and the soundness of $\prev$. For the cases $\iread$ and $\iend$ a second induction has to be done for distinguishing all different histories that can be generated when changing one $\wro$-edge or when swapping two events.
\end{proof}

\begin{corollary}
Every $\iread$ that is not swapped can read from any previous $\iwrite$.
\end{corollary}

\begin{corollary}
Let $h$ a history and $w \in \tr(\last{h})$ a $\iwrite$ event that writes $x \in \mathcal{V}$. For every $r$ event such that $\canSwap(w,r,h)$, the history $h'$ obtained by swapping $r$ and $w$ will be computed.
\end{corollary}
...
The base case, $h_{|D|}$ is trivial as by its definition it corresponds with $h'$. Let's prove the inductive case: $\{e \ | \ \swapped{h_{k+1}}{e}\} = \{e \ | \ \swapped{h'}{e}\}$. If $e_{k+1}$ is not a $\iread$ event, $h_k = h_{k+1} \bullet e_{k+1}$ and $\{e \ | \ \swapped{h_{k}}{e}\} = \{e \ | \ \swapped{h'}{e}\}$; as only $\iread$ events can be swapped. Otherwise, by the model's \textcolor{orange}{past-extensibility} there exists a $\iwrite$ event $f_{k+1}$ s.t. writes the same variable and $\isConsistent{h_{k+1} \bullet_{f_{k+1}} e_{k+1}} \land \{e \ | \ \swapped{h_{k+1}}{e}\} = \{e \ | \ \swapped{h_{k+1} \bullet_{f_{k+1}} e_{k+1}}{e}\}$ holds. Let $w_{k+1} = \max_{\leq^{h_{k+1}}} \{w \ | \ \isConsistent{h_{k+1} \bullet_{w} e_{k+1}} \land \{e \ | \ \swapped{h_{k+1}}{e}\} = \{e \ | \ \swapped{h_{k+1} \bullet_{w} e_{k+1}}{e}\}\}$. Therefore, $h_k = h_{k+1} \bullet_{w_{k+1}} e_{k´+1}$ is consistent and $\{e \ | \ \swapped{h_{k}}{e}\} = \{e \ | \ \swapped{h'}{e}\}$. Moreover, let's remark that as $w_{k+1}$ is the maximum write event according to $\leq_{h_{k+1}}$ s.t. $\isConsistent{h_k}$ and $\{e \ | \ \swapped{h_{k}}{e}\} = \{e \ | \ \swapped{h'}{e}\}$ and $R^{\ora}(h)$, it also satisfies $\isMaximallyAdded{h_{k}}{ e_{k+1}, w_{k+1}}$. Altogether, we obtain $h_p = h_0$; which let us conclude $\{e \in h_p \ | \ \swapped{h_p}{e}\} = \{e \in h' \ | \ \swapped{h'}{e}\} = \{e \in h \ | \ \swapped{h}{e}\}\setminus \{a\}$. \textcolor{orange}{In orange where I use the new constraint.}
\end{proof}
\end{lemma}
