...
\begin{lemma}
	...
	\begin{proof}
		We will prove it by induction on the number of \ref{algorithm:optimal-instantiated}'s stack calls a computable path that leads to a history $h$ needs, $n$. The base case, $n = 0$, is for the trivial history $h = \emptyset$ where it trivially holds; so let us prove the inductive case; being $e \gets \nextEvent(h)$. On one hand, $e$ is not a $\iread$ nor a $\ibegin$ event and $h' = h \bullet e$, as $\lnot \swapped{h}{e}$ and $h'$ is edge-wise identical to $h$, $R^{\ora}(h')$ holds. 
