...
We propose \emph{stateless} model checking algorithms for database-backed applications that rely on \emph{dynamic} partial order reduction~\cite{DBLP:conf/popl/FlanaganG05}. Stateless model checking explores executions of a program in a systematic manner without storing visited states, thereby, avoiding excessive memory consumption. Dynamic partial order reduction means that the independence relation between events is discovered on the fly without relying on a-priori static analyses.
