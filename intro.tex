...
While the mentioned works concern shared memory programs, we are not aware of any published work addressing the case of database transactional programs running under weak isolation levels. In this paper, we address this case and propose new stateless model checking algorithms relying on DPOR techniques for database-backed applications.  Our work generalizes the approach introduced by \cite{DBLP:journals/pacmpl/Kokologiannakis22}. However, this generalization to the transactional case, handling the most relevant isolation levels, is not a straightforward adaptation of \cite{DBLP:journals/pacmpl/Kokologiannakis22}. In particular, ensuring optimality while preserving the other properties, and especially completeness and polynomial memory complexity, turned out to be a challenging issue. We explain in the following the main steps and features of our work.  
