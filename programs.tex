...
The minimal element of $\po_t$ is a $\ebegin$ event. A transaction log without an $\eend$ event is called \emph{pending}. Otherwise, it is called \emph{complete}. If the $\eend$ event occurs, then it is maximal in $\po_t$. The set $E$ of events in a transaction log $t$ is denoted by $\events{t}$.
...
To simplify the notation, we extend $\so$ and $\wro$ to pairs of events by $(e_1,e_2)\in \so$ if $(\trans{h}{e_1},\trans{h}{e_2})\in\so$ and $(e_1,e_2)\in \wro$ if $e_1\in\writeOp{\trans{h}{e_1}}$, $(\trans{h}{e_1},e_2)\in\wro$, and $\mathit{var}(e_1)=\mathit{var}(e_2)$. We also define $\po=\bigcup_{t\in T} \po_t$.
